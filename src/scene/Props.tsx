...
import { liftForTexture, plankPixels, surfaceTexture } from './textures';
...
export function Props({ shell }: { shell: Shell }) {
  ...
  const plank = useMemo(
    () => surfaceTexture(plankPixels(0x91b0), [BENCH.length / 1.1, 1]),
    [],
  );
  useLayoutEffect(() => () => plank.dispose(), [plank]);
    ...
    <group position={[0, FLOOR_Y, 0]}>
      ...
      <group position={[benchX, 0, benchZ]}>
        <mesh position={[0, BENCH.height, 0]} castShadow receiveShadow>
          ...
          <meshStandardMaterial map={plank} color={wood} roughness={0.85} />
        </mesh>
        ...
        {[-0.22, 0, 0.22].map((z) => (
          <mesh key={z} position={[0, 0.32, z]} castShadow receiveShadow>
            ...
            <meshStandardMaterial map={plank} color={wood} roughness={0.9} />
          </mesh>
        ))}
